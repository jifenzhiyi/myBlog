...
class Shape {
  ...
  configureShapeType() {
    ...
    if (this.type === 'circle') {
      ...
      this.DOM.el.setAttribute('points', `${this.letterRect.left}
      ...
      if (config && Object.keys(config).length !== 0) {
        if (config.shapesAnimationOpts) {
          for (let i = 0, len = this.letters.length; i <= len - 1; i += 1) {
            ...
            }, config.lettersAnimationOpts && config.lettersAnimationOpts.delay ? config.lettersAnimationOpts.delay(letter.DOM.el, i) : 0);
          }
        }
  ...
  show(direction) {
    ...
    anime({
      ...
      complete: () => {
        ...
        this.words[newPos].show(effects[newPos].show).then(() => {
          this.isAnimating = false;
        });
      },
    });
