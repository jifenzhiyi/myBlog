  ...
  window.Meny = {
  	...
  	create: function( options ) {
  		return (function(){
  			...
  			function setupWrapper() {
  				...
  				// position
  				window.Meny.addClass( dom.wrapper, 'meny-' + config.position );
  				originalStyles.wrapper = dom.wrapper.style.cssText;
  				dom.wrapper.style[ window.Meny.prefix( 'perspective' ) ] = '800px';
  				dom.wrapper.style[ window.Meny.prefix( 'perspectiveOrigin' ) ] = contentsTransformOrigin;
  			}
  			/**
          ...
          dom.cover.style['border-radius'] = '0 20px 20px 0';
  				...
  				if( supports3DTransforms ) {
  					dom.cover.style[ window.Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
  				}
  			...
  			function setupMenu() {
  				...
  				style.display = 'block';
  				dom.menu.style.zIndex = 1;
  				if( supports3DTransforms ) {
  					style[ window.Meny.prefix( 'transform' ) ] = menuTransformClosed;
  					style[ window.Meny.prefix( 'transformOrigin' ) ] = menuTransformOrigin;
  					style[ window.Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
  				}
  				else {
  					window.Meny.extend( style, menuStyleClosed );
  				}
  			}
  			/**
  			...
  			function setupContents() {
  				...
  				if( supports3DTransforms ) {
  					style[ window.Meny.prefix( 'transform' ) ] = contentsTransformClosed;
  					style[ window.Meny.prefix( 'transformOrigin' ) ] = contentsTransformOrigin;
  					style[ window.Meny.prefix( 'transition' ) ] = 'all ' + config.transitionDuration +' '+ config.transitionEasing;
  				}
  				else {
  					style.position = style.position.match( /relative|absolute|fixed/gi ) ? style.position : 'relative';
  					window.Meny.extend( style, contentsStyleClosed );
  				}
  			}
  			/**
  			...
  			function bindEvents() {
  				if( 'ontouchstart' in window ) {
  					if( config.touch ) {
  						window.Meny.bindEvent( document, 'touchstart', onTouchStart );
  						window.Meny.bindEvent( document, 'touchend', onTouchEnd );
  					}
  					else {
  						window.Meny.unbindEvent( document, 'touchstart', onTouchStart );
  						window.Meny.unbindEvent( document, 'touchend', onTouchEnd );
  					}
  				}
  				if( config.mouse ) {
  					window.Meny.bindEvent( document, 'mousedown', onMouseDown );
  					window.Meny.bindEvent( document, 'mouseup', onMouseUp );
  					window.Meny.bindEvent( document, 'mousemove', onMouseMove );
  				}
  				else {
  					window.Meny.unbindEvent( document, 'mousedown', onMouseDown );
  					window.Meny.unbindEvent( document, 'mouseup', onMouseUp );
  					window.Meny.unbindEvent( document, 'mousemove', onMouseMove );
  				}
  			}
  			/**
  			 * Expands the menu.
  			...
  			function open() {
  				if( !isOpen ) {
  					isOpen = true;
  					window.Meny.addClass( dom.wrapper, 'meny-active' );
  					...
  					if( supports3DTransforms ) {
  						...
  						window.Meny.bindEventOnce( dom.wrapper, 'transitionend', function() {
  							window.Meny.dispatchEvent( dom.menu, 'opened' );
  						} );
  						dom.cover.style.opacity = 1;
  						dom.contents.style[ window.Meny.prefix( 'transform' ) ] = contentsTransformOpened;
              dom.menu.style[ window.Meny.prefix( 'transform' ) ] = menuTransformOpened;

              if (document.querySelector(".meny-arrow")) {
                document.querySelector(".meny-arrow").style.display = 'none';
              }
              var domfixed = dom.menu;
              var menyContent= dom.contents;
              ...
              menyContent.className = menyContent.className + " menu-open";
  					...
  					// ...fall back on JS animation
  						...
  						menuAnimation = window.Meny.animate( dom.menu, menuStyleOpened, 500 );
  						contentsAnimation && contentsAnimation.stop();
  						contentsAnimation = window.Meny.animate( dom.contents, contentsStyleOpened, 500 );
  						coverAnimation && coverAnimation.stop();
  						coverAnimation = window.Meny.animate( dom.cover, { opacity: 1 }, 500 );
  					}
            window.Meny.dispatchEvent( dom.menu, 'open' );
  				}
  			}
  			/**
  			 * Collapses the menu.
          ...
          // $(".meny-arrow").show();
  				if( isOpen ) {
  					isOpen = false;
  					window.Meny.removeClass( dom.wrapper, 'meny-active' );
  					// Use transforms and transitions if available...
  					if( supports3DTransforms ) {
  						// 'webkitAnimationEnd oanimationend msAnimationEnd animationend transitionend'
  						window.Meny.bindEventOnce( dom.wrapper, 'transitionend', function() {
  							window.Meny.dispatchEvent( dom.menu, 'closed' );
  						} );
  						dom.cover.style.visibility = 'hidden';
  						dom.cover.style.opacity = 0;
  						dom.contents.style[ window.Meny.prefix( 'transform' ) ] = contentsTransformClosed;
              dom.menu.style[ window.Meny.prefix( 'transform' ) ] = menuTransformClosed;
              if (document.querySelector(".meny-arrow")) {
                document.querySelector(".meny-arrow").style.display = 'block';
              }
              var domfixed = dom.menu;
              var menyContent= dom.contents;
              ...
              menyContent.className = " meny-contents";
  					...
  					// ...fall back on JS animation
  						...
  						menuAnimation && menuAnimation.stop();
  						menuAnimation = window.Meny.animate( dom.menu, menuStyleClosed, 500 );
  						contentsAnimation && contentsAnimation.stop();
  						contentsAnimation = window.Meny.animate( dom.contents, contentsStyleClosed, 500 );
  						coverAnimation && coverAnimation.stop();
  						coverAnimation = window.Meny.animate( dom.cover, { opacity: 0 }, 500, function() {
  							dom.cover.style.visibility = 'hidden';
  							window.Meny.dispatchEvent( dom.menu, 'closed' );
  						} );
  					}
  					window.Meny.dispatchEvent( dom.menu, 'close' );
  				}
  			}
  			/**
  			...
  			function onTouchStart( event ) {
  				...
  				touchMoveX = null;
  				touchMoveY = null;
  				window.Meny.bindEvent( document, 'touchmove', onTouchMove );
  			}
  			...
  			function onTouchEnd( event ) {
  				window.Meny.unbindEvent( document, 'touchmove', onTouchMove );
  			...
  			return {
  				...
  				addEventListener: function( type, listener ) {
  					...
  					dom.menu && window.Meny.bindEvent( dom.menu, type, listener );
  				},
  				removeEventListener: function( type, listener ) {
  					dom.menu && window.Meny.unbindEvent( dom.menu, type, listener );
  				}
  			};
  		})();
  	},
  	...
  	dispatchEvent: function( element, type, properties ) {
  		if( element ) {
  			...
  			window.Meny.extend( event, properties );
